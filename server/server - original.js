...
router.get('/player', async function(req, res){
 data = service.getPlayers(req.query.gameId);
 res.json(await data);  
})
...
router.post('/player', function(req, res) {
  data = service.setPlayers(req.query.gameId);
  res.json(data);
})

router.post('/weapon', function(req, res) {
  data = service.getPlayers;
  if (data.players) {
    ...
    service.setPlayers();
    //Set player values
    GameModel.findOneAndUpdate({"_id":req.query.gameID}, { $set: {playerId: req.query.playerId, playerWeapon: req.query.weapon},
      function (err, game) {
        if (err) {
