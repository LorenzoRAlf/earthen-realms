...
public class Treasury extends JFrame {
	...
	public Treasury(String idPlayer) {
		...
		JLabel lblNewLabel = new JLabel("BIENVENIDO A LA TESORER\u00CDA");
		...
		JLabel coins = new JLabel(coinsd.coins + " monedas");
		...
		btnArma.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				int monedas =Integer.parseInt(coins.getText().replaceAll(" monedas", ""));
				if (monedas < 99) {
					JOptionPane.showMessageDialog(null, "No tens suficients monedes");
				} else {
					Random r = new Random();
					int low = 0;
					...
					try {
						if (tipoa.equals("Electric")) {
							...
							armaR.setHorizontalAlignment(SwingConstants.CENTER);
							armaR.setBounds(0, 11, 324, 72);
							armaR.setIcon(new ImageIcon(new URL("http://i.stack.imgur.com/UvHN4.png")));
							panel_3.add(armaR);
							repaint();
					...
					if (!hasit) {
						coins.setText(monedas - 100 + " monedas");
						repaint();
					}
					hasit = false;
				}
...
//		}
		...
		JLabel lblNewLabel_4 = new JLabel("Armaduras");
